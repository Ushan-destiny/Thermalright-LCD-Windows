...
class Controller:
    def __init__(self, config_path=None):
        ...
        if config_path is None:
            # FIX: Look in the CURRENT folder (src), not the parent
            self.config_path = os.environ.get('DIGITAL_LCD_CONFIG', os.path.join(os.path.dirname(os.path.abspath(__file__)), 'config.json'))
        else:
    ...
    def get_device(self):
        try:
            # FIX: Using 'hidapi' library syntax instead of 'hid'
            h = hid.device()
    ...
    def send_packets(self):
        ...
        # FIX: Prepend 00 for Windows HID Report ID
        packet0 = bytes.fromhex("00" + self.HEADER + message[:128-len(self.HEADER)])
    ...
    def display(self):
        while True:
            ...
            if self.dev is None:
                ...
                print(f"Sending command... Mode: {self.display_mode}") # <--- ADD THIS LINE
